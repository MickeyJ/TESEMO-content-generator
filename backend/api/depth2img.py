...
class Depth2ImgPipeline(DiffusionPipeline):
    ...
    def get_depth_mask(self, img):
        ...
        pixel_values = self.depth_feature_extractor(
...
vae = AutoencoderKL.from_pretrained(
    "stabilityai/stable-diffusion-2-depth", subfolder="vae"
).to(device)
# Load tokenizer and the text encoder
tokenizer = CLIPTokenizer.from_pretrained(
    "stabilityai/stable-diffusion-2-depth", subfolder="tokenizer"
)
text_encoder = CLIPTextModel.from_pretrained(
    "stabilityai/stable-diffusion-2-depth", subfolder="text_encoder"
).to(device)
# Load UNet model
unet = UNet2DConditionModel.from_pretrained(
    "stabilityai/stable-diffusion-2-depth", subfolder="unet"
).to(device)
# Load scheduler
...
depth_estimator = DPTForDepthEstimation.from_pretrained(
    "stabilityai/stable-diffusion-2-depth", subfolder="depth_estimator"
)
# Load DPT Feature Extractor
depth_feature_extractor = DPTFeatureExtractor.from_pretrained(
    "stabilityai/stable-diffusion-2-depth", subfolder="feature_extractor"
)


depth2img = Depth2ImgPipeline(
    vae,
    tokenizer,
    text_encoder,
    unet,
